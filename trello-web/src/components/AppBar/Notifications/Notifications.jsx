...
function Notifications() {
  ...
  //state kiem tra co thong bao moi hay khong
  ...
  const handleClickNotificationIcon = (event) => {
    ...
    //khi click vao tbao thi set lai trang thai noti ve false
  ...
  //fetch danh sach cac loi moi
  useEffect(() => {
    dispatch(fetchInvitationsAPI())


    // tao 1 func xu ly khi nhan su kien real-time tu server
    const onReceiveNewInvitation = (invitation) => {
      //neu user dang dang nhap la nguoi duoc moi thi hien thong bao
      if (invitation.inviteeId === currentUser._id) {
        //buoc 1: them thong bao moi vao redux
        dispatch(addNotification(invitation))
        //buoc 2: thiet lap co thong bao moi
        setNewNotification(true)
      }
    }

    // lang nghe su kien co ten la BE_USER_INVITED_TO_BOARD
    socketIoInstance.on('BE_USER_INVITED_TO_BOARD', onReceiveNewInvitation)

    // clean up de ngan khong lang nghe su kien nay nhieu lan
    return () => {
